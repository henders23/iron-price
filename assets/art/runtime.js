(function () {
  ...
  function endingScene(text, victory) {
    ...
    if (normalized.includes("closed every road")) return "ending-ironbound-threat";
  ...
  function weaponName(text) {
    const normalized = text.toLowerCase();
    return ["sword", "spear", "axe", "mace"].find((name) => normalized.includes(name));
  }
  ...
  function qualityFrom(rootElement) {
    const text = rootElement?.querySelector(".section-label span")?.textContent || rootElement?.textContent || "";
  ...
  function enhance() {
    ...
    if (eventScreen) {
      const kind = Object.keys(eventScenes).find((name) => eventScreen.classList.contains(`event-${name}`));
    ...
    if (ending) {
      const scene = endingScene(ending.querySelector(".ending-card > p")?.textContent || "", ending.classList.contains("victory"));
      ending.classList.add("ip-ending-art");
      setArt(ending, "--ip-scene", `scenes/${scene}.webp`);
      setEmblem(ending.querySelector(".ending-seal"), ending.classList.contains("victory") ? "frontier-compact" : "company");
    }

    setEmblem(document.querySelector(".campaign-brand .brand-rivet"), "company");

    document.querySelectorAll(".supply-strip > div").forEach((item) => {
      const name = item.title?.toLowerCase();
      const icon = resourceIcons[name];
      if (icon) setIcon(item.querySelector("i"), `icons/resources/${icon}.webp`);
    });

    document.querySelectorAll(".market-card").forEach((card) => {
      const className = [...card.classList].find((name) => name.startsWith("offer-"));
      const type = className?.slice(6);
      const quality = Number(card.textContent.match(/QUALITY\s+(\d)/i)?.[1] || 1);
      const icon = card.querySelector(".market-icon");
      if (type === "weapon") setIcon(icon, `icons/weapons/sword-${weaponTier(quality)}.webp`);
      else if (type === "armor") setIcon(icon, "icons/resources/body-armor.webp");
      else if (["food", "tools", "medicine"].includes(type)) setIcon(icon, `icons/resources/${type}.webp`);
    });

    document.querySelectorAll(".armory-options button[data-equip-weapon]").forEach((button) => {
      const quality = qualityFrom(button.closest(".loadout-section"));
      setIcon(button.querySelector("i"), `icons/weapons/${button.dataset.equipWeapon}-${weaponTier(quality)}.webp`);
    });

    document.querySelectorAll(".recruit-kit").forEach((kit) => {
      const weapon = weaponName(kit.textContent);
      if (weapon) setIcon(kit.querySelector("span"), `icons/weapons/${weapon}-worn.webp`);
    });

    document.querySelectorAll(".contract-card").forEach((card) => {
      const normalized = card.textContent.toLowerCase();
      const faction = Object.entries(factionEmblems).find(([name]) => normalized.includes(name))?.[1];
    ...
    document.querySelectorAll(".sheet-resource").forEach((row) => {
      ...
      const icon = label === "head" ? "head-armor" : label === "body" ? "body-armor" : null;
      if (icon) {
