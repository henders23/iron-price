(() => {
  ...
  const ipArtCache = new Map();
  function ipArtImage(e) {
    if (!ipArtCache.has(e)) {
      const t = new Image();
      ((t.decoding = "async"),
        (t.src = `./assets/art/${e}`),
    ...
    return ipArtCache.get(e);
  }
  function ipDrawArt(e, t, a, i, n, r, o = 1) {
    const l = ipArtImage(t);
    l.complete &&
      l.naturalWidth &&
  ...
  const k = {
    ...
    W = ["fleeing", "breaking", "wavering", "steady", "confident"];
  function Z(s) {
    return W.indexOf(s);
  }
  function Ee(s) {
    return W[Math.max(0, Z(s) - 1)] ?? "fleeing";
  }
  const Re = [
    { q: 1, r: 0 },
    { q: 1, r: -1 },
  ...
  function A(s, e) {
    return s.find((t) => q(t, e));
  }
  function M(s, e, t = !1) {
    return s.find((a) => (t || (a.alive && !a.routed)) && q(a.position, e));
  }
  function V(s, e, t, a, i = Number.POSITIVE_INFINITY) {
    if (q(t, a)) return [t];
    const n = A(s, a);
    if (!n || C[n.terrain].moveCost >= 99) return null;
    const r = M(e, a);
    if (r && !q(r.position, t)) return null;
    const o = [{ hex: t, cost: 0 }],
      l = new Map([[L(t), null]]),
      d = new Map([[L(t), 0]]),
      c = new Map([[L(t), t]]);
    for (; o.length > 0;) {
      o.sort((y, u) => y.cost + E(y.hex, a) - (u.cost + E(u.hex, a)));
      const f = o.shift();
      if (!f || q(f.hex, a)) break;
      for (const y of J(f.hex)) {
        const u = A(s, y);
        if (!u || C[u.terrain].moveCost >= 99 || (M(e, y) && !q(y, a)))
          continue;
        const g =
            C[u.terrain].moveCost +
            Math.max(0, u.elevation - (A(s, f.hex)?.elevation ?? 0)),
          T = f.cost + g;
        if (T > i) continue;
        const x = L(y);
  ...
  function ye(s, e) {
    ...
    for (let a = 1; a < e.length; a += 1) {
      ...
      if (!i) return 999;
      t +=
        C[i.terrain].moveCost + Math.max(0, i.elevation - (n?.elevation ?? 0));
    }
    return t;
  }
  function G(s, e, t) {
    const a = new Map();
    for (const i of s) {
      const n = V(s, e, t.position, i, t.ap);
      n && a.set(L(i), n);
    }
    return a;
  }
  ...
  function Y(s) {
    return (Z(s) - Z("steady")) * 5;
  }
  function le(s) {
    return s.initiative - Math.floor(s.fatigue * 0.45) + Y(s.morale);
  }
  function we(s) {
  ...
  function $e(s, e) {
    ...
    ((s.queue = s.units
      ...
      .sort((t, a) => le(a) - le(t) || t.id.localeCompare(a.id))
      .map((t) => t.id)),
      e.push({ type: "roundStarted", round: s.round }));
  }
  function ee(s, e) {
    const t = de(s, "company"),
      a = de(s, "enemy"),
      i = s.objective;
    if (i?.type === "break-captain") {
      const n = s.units.find((r) => r.id === i.captainId);
      if (!n || !n.alive || n.routed) return N(s, e, "company");
    }
    if (
      i?.type === "escape" &&
      (s.objectiveState?.escapedCompanyIds.length ?? 0) >= (i.escapeCount ?? 3)
    )
      return N(s, e, "company");
    if (t.length === 0) return N(s, e, "enemy");
    if (a.length === 0) return N(s, e, "company");
  }
  function Oe(s, e) {
    const t = s.objective,
      a = s.objectiveState;
    if (!(!t || !a || s.phase !== "battle")) {
      ...
      if (t.type === "hold" && t.holdHex) {
        ...
        ((a.heldRounds =
          ...
          a.heldRounds >= (t.holdRounds ?? 2) && N(s, e, "company"));
      }
    }
  }
  function ce(s, e) {
  ...
  function De(s, e, t) {
    return J(t.position).filter((a) => {
      const i = M(s.units, a);
      return i?.team === e.team && i.alive && !i.routed;
    }).length;
  }
  function ae(s, e, t) {
    const a = k[e.weaponId],
      i = [
    ...
    const n = A(s.tiles, t.position),
      r = A(s.tiles, e.position);
    if (n) {
      const v = C[n.terrain].defense;
      v !== 0 && i.push({ label: C[n.terrain].name, value: -v });
    }
  ...
  function he(s, e, t, a, i = 0) {
    if (!e.alive || e.routed || e.morale === "fleeing") return;
    let n;
    ({ value: n, state: s.rngState } = z(s.rngState, 1, 100));
    const r = e.resolve + Y(e.morale) - i;
    if (n <= r) return;
    const o = e.morale,
      l = Ee(o);
    ((e.morale = l),
  ...
  function ke(s, e) {
    const t = Pe(s),
      a = [];
    if (e.type === "deploy") {
      if (t.phase !== "deployment") return w(s, "Deployment is already over.");
      const r = t.units.find((c) => c.id === e.unitId && c.team === "company");
      if (!r) return w(s, "Only company fighters can be deployed.");
      const o = A(t.tiles, e.destination),
        l = e.destination.q + Math.floor(e.destination.r / 2);
      if (!o || l > 3 || C[o.terrain].moveCost >= 99)
        return w(s, "Choose an open hex inside the blue deployment zone.");
      if (M(t.units, e.destination)) return w(s, "That hex is occupied.");
      const d = { ...r.position };
      return (
    ...
    if (e.type === "startBattle")
      return t.phase !== "deployment"
        ? w(s, "The battle is already underway.")
        : ((t.phase = "battle"),
          (t.commandCount += 1),
          ...
          { state: t, events: a });
    const i = je(t, e.unitId);
    if (typeof i == "string") return w(s, i);
    const n = i;
    if (e.type === "move") {
      const r = e.path.at(-1);
      if (!r) return w(s, "Choose a destination.");
      const o = V(t.tiles, t.units, n.position, r, n.ap);
      if (!o) return w(s, "No legal path reaches that hex.");
      const l = ye(t.tiles, o),
        d = l * 3;
      return l <= 0 || l > n.ap
        ? w(s, "Not enough action points.")
        : n.fatigue + d > n.fatigueMax
          ? w(s, "Too exhausted to move that far.")
          : ((n.position = { ...r }),
            (n.ap -= l),
            (n.fatigue += d),
            (n.movedThisTurn = !0),
    ...
    if (e.type === "attack") {
      ...
      if (!r || !r.alive || r.routed || r.team === n.team)
        return w(s, "Choose a living enemy.");
      if (E(n.position, r.position) !== 1)
        return w(s, "The target is out of reach.");
      const o = k[n.weaponId];
      if (n.ap < o.apCost) return w(s, "Not enough action points.");
      if (n.fatigue + o.fatigueCost > n.fatigueMax)
        return w(s, "Too exhausted to attack.");
      ((n.ap -= o.apCost), (n.fatigue += o.fatigueCost), (t.commandCount += 1));
      const l = ae(t, n, r);
      let d;
      ({ value: d, state: t.rngState } = z(t.rngState, 1, 100));
      const c = d <= l.chance;
      if (
        (a.push({
          type: "attackRolled",
      ...
      let m;
      ({ value: m, state: t.rngState } = z(t.rngState, 1, 100));
      const v = m <= 24 ? "head" : "body";
      let b;
      ({ value: b, state: t.rngState } = z(
        t.rngState,
        o.damageMin,
        o.damageMax,
      ));
      const f = v === "head" ? "headArmor" : "bodyArmor",
        ...
        T = Math.min(
          ...
          Math.max(
            1,
            Math.floor(
              b * o.penetration + g / Math.max(0.5, o.armorMultiplier),
            ),
          ),
        );
      if (
        ...
        for (const P of x) he(t, P, `${r.name} fell`, a, 12);
        ee(t, a);
      } else
        (T >= 15 || r.hp / r.hpMax < 0.35) &&
          he(t, r, `struck by ${o.name}`, a, T >= 24 ? 15 : 7);
      return { state: t, events: a };
    }
    return e.type === "wait"
      ? n.waited || t.queue.length <= 1
        ? w(s, "This fighter cannot wait again.")
        : ((n.waited = !0),
          t.queue.shift(),
          t.queue.push(n.id),
          (t.selectedUnitId = t.queue[0] ?? null),
  ...
  function Ue(s, e) {
    return se(s, e).filter((t) => E(e.position, t.position) === 1);
  }
  function ue(s, e, t) {
    const a = ae(s, e, t),
  ...
  function Be(s, e) {
    const t = k[e.weaponId];
    if (e.ap < t.apCost || e.fatigue + t.fatigueCost > e.fatigueMax)
      return null;
    const a = Ue(s, e);
    return a.length === 0
      ? null
      : (a.sort(
          (i, n) => ue(s, e, n) - ue(s, e, i) || i.id.localeCompare(n.id),
        ),
        { type: "attack", unitId: e.id, targetId: a[0].id });
  }
  function ze(s, e, t) {
    return J(t).reduce((a, i) => {
      const n = M(s.units, i);
  ...
  function Ve(s, e) {
    ...
    const n = k[e.weaponId],
      r = i >= n.apCost + 1 ? n.apCost : 0;
    let o = G(s.tiles, s.units, { ...e, ap: i - r });
    o.size <= 1 && r > 0 && (o = G(s.tiles, s.units, { ...e, ap: i }));
    const d = [...o.entries()]
      .map(([c, m]) => {
        const v = A(s.tiles, m.at(-1) ?? e.position),
          b = m.at(-1) ?? e.position,
          f = Math.min(...t.map((x) => E(b, x.position))),
          y = ze(s, e, b),
          u = We(s, e, b),
          p = v ? C[v.terrain] : C.grass,
          g = v?.elevation ?? 0,
          T =
            -f * 25 +
            g * 7 +
  ...
  function Ge(s, e) {
    const t = se(s, e),
      ...
      n = [...G(s.tiles, s.units, { ...e, ap: a }).entries()]
        .map(([r, o]) => {
          const l = o.at(-1) ?? e.position,
            d = l.q + Math.floor(l.r / 2),
            c = e.team === "company" ? -d : d,
            m = t.length > 0 ? Math.min(...t.map((v) => E(l, v.position))) : 0;
  ...
  function Ye(s) {
    ...
    if (!e) return null;
    if (e.morale === "fleeing")
      return Ge(s, e) ?? { type: "endTurn", unitId: e.id };
    const t = Be(s, e);
    if (t) return t;
    const a = Ve(s, e);
  ...
  class it {
    ...
    resize() {
      ...
      const t = (this.width - 54) / (12.5 * O),
        a = (this.height - 58) / 14.3;
      this.size = Math.max(25, Math.min(49, t, a));
      const i = O * this.size * 12.5,
        n = this.size * 14.5;
      this.origin = {
        x: (this.width - i) / 2 + this.size * O * 0.5,
    ...
    hexCenter(e) {
      return {
        x: this.origin.x + this.size * O * (e.q + e.r / 2),
    ...
    drawTiles(e) {
      ...
      for (const a of this.state.tiles) {
        ...
        (r.addColorStop(0, H(n.color, a.elevation ? 20 : 10)),
          r.addColorStop(1, H(n.color, -14)),
          (t.fillStyle = r),
          t.fill(),
        ...
        if (
          ...
          this.view.reachable.has(l) &&
            ...
            (t.lineWidth = 1.4),
            t.stroke()),
          this.view.hovered && q(a, this.view.hovered))
        ) {
          this.drawHex(i, this.size * 0.82);
          const m = M(this.state.units, a)?.team === "enemy";
        ...
        (this.state.objective?.type === "hold" &&
          ...
          q(a, this.state.objective.holdHex) &&
    ...
    drawUnit(e, t) {
      const a = this.context,
        ...
        n = this.size / 42,
        r =
          e.id === this.view.selectedUnitId ||
          e.id === this.view.inspectedUnitId,
      ...
      (f.addColorStop(0, H(v, Math.floor(b * 24))),
        f.addColorStop(1, H(v, -18)),
        (a.fillStyle = f),
  ...
  function lt(s, e) {
    const t = pe[$(s + e * 101) % pe.length],
      a = ["spear", "sword", "mace", "axe"],
      i = a[$(s + e * 211) % a.length],
      n = $(s + e * 307) % 12;
    return {
      id: Ce(s + 2748, e),
      name: t[0],
      ...
      bodyArmor: 44 + n * 2,
      bodyArmorMax: 44 + n * 2,
      initiative: 94 + ($(s + e * 401) % 24),
      meleeSkill: 58 + ($(s + e * 503) % 9),
      meleeDefense: 2 + ($(s + e * 601) % 6),
      resolve: 43 + ($(s + e * 701) % 15),
      weaponId: i,
      hasShield: i === "spear" || i === "sword",
      wage: 10 + ($(s + e * 809) % 7),
      hireCost: 135 + ($(s + e * 907) % 130),
      alive: !0,
      injuries: [],
      kills: 0,
      battles: 0,
  ...
  function ie(s) {
    return rt
      .filter((e) => e.locationId === s.currentLocation)
  ...
  function Me(s) {
    return Array.from({ length: 3 }, (e, t) =>
      lt(s.baseSeed + s.day * 1297, t),
    );
  }
  function ne(s) {
    const e = Math.min(3, 2 + Math.floor(s.completedContracts / 4)),
      t = $(s.baseSeed + s.day * 1999 + s.currentLocation.length * 811),
  ...
  function dt(s) {
    return structuredClone(
      me[$(s.baseSeed + s.day * 3571 + s.currentLocation.length) % me.length],
    );
  }
  function ct(s, e, t, a) {
    const i = $(Date.now() + s * 10007),
      n = {
        version: 2,
        slot: s,
    ...
    return ((n.recruits = Me(n)), (n.contracts = ie(n)), (n.market = ne(n)), n);
  }
  function ve(s) {
          ...
          ? s.food < i
            ...
            : s.crowns < n
              ...
              : ((s.day += 2),
                ...
                (s.pendingEvent = dt(s)),
                s.history.push(
                  `Day ${s.day}: the company reached ${S[e].name}.`,
                ),
                re(s),
                B(s),
                {
                  success: !0,
                  message: `${S[e].name} reached. ${n} crowns and ${i} food spent.`,
                })
          : {
              success: !1,
  ...
  function ut(s, e, t) {
    ...
    if (!a)
      return { success: !1, message: "That offer is no longer available." };
    if (s.crowns < a.cost)
    ...
    return (
      ...
      s.history.push(
        `${a.name} purchased in ${S[s.currentLocation].name} for ${a.cost} crowns.`,
      ),
  ...
  function pt(s, e) {
    ...
    if (t.id === "broken-cart" && e === "help")
      (s.tools > 0 && (s.tools -= 1),
        ...
        (a =
          "The merchant pays in provisions and a useful word ahead of you."));
      ...
      ((s.medicine += 3),
        (s.threat += 4),
        (a =
          "The medicine is good. Whoever owned it now follows your tracks."));
    else if (t.id === "old-fire")
      ((s.day += 1),
        (s.renown += 2),
        (s.reputations[S[s.currentLocation].reputation] += 2),
        (a =
  ...
  function mt(s, e, t) {
    ...
    return !a || a.perkPoints <= 0 || a.perks.includes(t)
      ...
      : ((a.perkPoints -= 1),
        ...
        t === "pathfinder" &&
          ((a.initiative += 10), (a.hpMax += 4), (a.hp += 4)),
        t === "quartermaster" && (a.wage = Math.max(5, a.wage - 3)),
        s.history.push(`${a.name} learned ${D[t].name}.`),
        !0);
  }
  function vt(s, e, t) {
    const a = s.roster.find((i) => i.id === e && i.alive);
  ...
  function gt(s) {
    const e = s.roster.filter((r) => r.alive),
      t = e.reduce(
        (r, o) =>
          r + o.headArmorMax - o.headArmor + o.bodyArmorMax - o.bodyArmor,
        0,
      );
    if (t <= 0)
      return {
  ...
  function kt(s, e, t, a) {
    ...
    for (const u of c) {
      ...
      if (p) {
        ...
        if ((u.routed && r.push(p.name), u.hp / u.hpMax < 0.42 || u.routed)) {
          const g = be[$(e.seed + p.id.length + p.battles * 17) % be.length];
    ...
    if (
      ...
      const u = ["sword", "spear", "axe", "mace"],
        p = u[$(e.seed + s.day) % u.length];
      ((s.armory[p] += 1), l.push(k[p].name));
    }
    return (
  ...
  function I(s) {
    ((s.lastPlayed = new Date().toISOString()),
      K.set(s.slot, structuredClone(s)),
      _()?.setItem(oe(s.slot), JSON.stringify(s)));
  }
  function Ae(s) {
    const e = _(),
      t = e?.getItem(oe(s)) ?? e?.getItem(xe(s));
    if (t)
      try {
        const a = ve(JSON.parse(t));
        if (a) return (I(a), a);
      } catch {
        return null;
      }
    return K.has(s) ? ve(K.get(s)) : null;
  }
  function St() {
    return [1, 2, 3].map(Ae);
  }
  ...
  <main class="game-shell">
    <header class="topbar">
      <div class="brand-block">
        ...
        <div><h1>IRON PRICE</h1><span>${e ? S[e.locationId].name.toUpperCase() : "THE ASH ROAD"}</span></div>
      </div>
...
`;
  ...
  class fe {
    ...
    constructor(e) {
      ((this.options = e),
        (this.seed = e.seed >>> 0),
        (this.initialState = structuredClone(e.initialState ?? Q(this.seed))),
    ...
    reset(e) {
      ((this.seed = e >>> 0),
        (this.state =
          e === this.seed ? structuredClone(this.initialState) : Q(e)),
        (this.commands = []),
    ...
    onHover(e) {
      if (
        ...
        t?.team === "company" &&
          !M(this.state.units, e) &&
          (this.previewPath =
            V(this.state.tiles, this.state.units, t.position, e, t.ap) ?? []);
      }
      this.refresh(!1);
    }
    async onBoardClick(e) {
      if (this.locked) return;
      const t = M(this.state.units, e, !0);
      ...
      if (!(!a || a.team !== "company" || a.morale === "fleeing"))
        if (t?.team === "enemy" && t.alive && E(a.position, t.position) === 1)
          await this.playerCommand({
            type: "attack",
            unitId: a.id,
            targetId: t.id,
          ...
          const i = V(this.state.tiles, this.state.units, a.position, e, a.ap);
          i &&
            i.length > 1 &&
      ...
      for (const t of e) {
        ...
        if (
          ...
          i &&
            n &&
            this.logEntries.push(
              `${i.name} ${k[t.weaponId].verb.toLowerCase()}s at ${n.name}: ${t.roll} vs ${t.chance}% — ${t.hit ? "hit" : "miss"}.`,
            );
        }
        if (t.type === "damaged") {
        ...
        (t.type === "moraleChanged" &&
          ...
          t.type === "unitDied" &&
            a &&
            this.logEntries.push(`${a.name} falls.`),
          t.type === "unitRouted" &&
            a &&
    ...
    refresh(e = !0) {
      ...
      t?.team === "company" &&
        this.state.phase === "battle" &&
        !this.locked &&
        (a = new Set(G(this.state.tiles, this.state.units, t).keys()));
      const i = this.hoveredHex
          ? M(this.state.units, this.hoveredHex, !0)
          : void 0,
        n =
          this.state.phase === "deployment"
            ? this.selectedDeploymentUnitId
      ...
      ((document.querySelector("#round-value").textContent =
        ...
        (document.querySelector("#hash-value").textContent = U(this.state)));
      ...
      ((e.innerHTML = this.state.queue
        ...
        e.querySelectorAll("[data-inspect]").forEach((t) =>
          t.addEventListener("click", () => {
            ((this.inspectedUnitId = t.dataset.inspect ?? null),
              this.refresh());
          }),
        ));
    }
    unitForPanel() {
      ...
      if (this.hoveredHex) {
        const e = M(this.state.units, this.hoveredHex, !0);
        if (e) return e;
      }
    ...
    refreshUnitPanel() {
      ...
      <div class="morale-track" aria-label="Morale: ${e.morale}">${W.slice()
        .reverse()
        .map(
          (n, r) =>
    ...
    refreshForecast() {
      const e = document.querySelector("#forecast-panel"),
        t = R(this.state),
        a = this.hoveredHex ? M(this.state.units, this.hoveredHex, !0) : void 0;
      if (this.state.phase === "deployment") {
        const i = this.hoveredHex
          ? A(this.state.tiles, this.hoveredHex)
          : void 0;
        e.innerHTML = `<div class="panel-heading"><span>FIELD READ</span></div>${i ? `<div class="terrain-read"><strong>${C[i.terrain].name}</strong><span>Move ${C[i.terrain].moveCost} AP · Defence ${C[i.terrain].defense >= 0 ? "+" : ""}${C[i.terrain].defense}${i.elevation ? " · High ground" : ""}</span></div>` : '<p class="muted">Hover a hex to inspect terrain.</p>'}`;
        return;
      }
      if (
        t &&
        ...
        t.team !== a.team &&
        a.alive &&
        E(t.position, a.position) === 1
      ) {
        const i = ae(this.state, t, a);
        e.innerHTML = `<div class="panel-heading"><span>ATTACK FORECAST</span><strong>${i.chance}%</strong></div>
        <div class="forecast-target"><span>${k[t.weaponId].verb}</span><b>${a.name}</b></div>
      ...
      if (t && this.previewPath.length > 1) {
        const i = ye(this.state.tiles, this.previewPath);
    ...
    refreshActions() {
      ...
      if (this.state.phase === "deployment") {
        ...
        ((t.innerHTML = `<span class="eyebrow">SELECTED FOR DEPLOYMENT</span><strong>${l.name}</strong><small>${k[l.weaponId].name}${l.hasShield ? " + shield" : ""}</small>`),
          (e.innerHTML =
      ...
      <button class="action-button end" id="end-action"><span class="action-icon">◆</span><span><b>End turn</b><small>${i.ap >= 4 ? "Recover 5 fatigue" : "Pass activation"}</small></span><kbd>E</kbd></button>`),
        e.querySelector("#attack-action")?.addEventListener("click", () => {
          const l = r.sort(
            (d, c) => d.hp - c.hp || d.id.localeCompare(c.id),
          )[0];
          l &&
            this.playerCommand({
              type: "attack",
    ...
    showResult() {
      const e = this.state.phase === "victory",
        ...
        r = at(this.seed, this.commands, this.state),
        o = U(tt(r)) === r.finalHash;
      ((document.querySelector("#result-kicker").textContent = e
        ...
        (document.querySelector("#result-stats").innerHTML =
          `<div><span>ROUNDS</span><b>${this.state.round}</b></div><div><span>COMPANY LOST</span><b>${i}/6</b></div><div><span>ENEMY BROKEN</span><b>${n}/6</b></div><div><span>REPLAY</span><b class="${o ? "verified" : "failed"}">${o ? "VERIFIED" : "FAILED"}</b></div><small>Seed ${this.seed.toString(16).toUpperCase()} · ${this.commands.length} commands · state ${U(this.state)}</small>`),
  ...
  class xt {
        ...
        document
          ...
          ?.addEventListener("click", () => {
            const t = document.querySelector("#company-name"),
              a = ct(e, t.value, this.pendingOrigin);
            (I(a), this.openCampaign(a));
          }));
    }
        ...
        <div class="campaign-body">
          <aside class="campaign-sidebar">
            <nav class="campaign-nav">
              <button data-view="map" class="${this.view === "map" ? "active" : ""}"><i>⌖</i><span>Frontier</span><small>${S[e.currentLocation].name}</small></button>
            ...
            <section class="camp-actions">
              ...
              <button id="rest-company"><b>Rest one day</b><small>−${j(e)} crowns · −${t.length} food</small></button>
    ...
    renderMarket() {
      ...
      return t.services.includes("market")
        ? `<div class="content-heading"><div><span class="eyebrow">MARKET · ${h(t.name.toUpperCase())}</span><h2>Steel, provisions and necessary lies</h2></div><div class="heading-stat"><span>COMPANY BALANCE</span><b>${e.crowns} crowns</b></div></div>
      <div class="market-target"><label><span>FIGHTER FOR EQUIPMENT REFITS</span><select id="market-fighter">${a.map((i) => `<option value="${i.id}">${h(i.name)} · Q${i.weaponQuality} weapon · Q${i.armorQuality} armor</option>`).join("")}</select></label></div>
      <div class="market-grid">${e.market.length ? e.market.map((i) => `<article class="market-card offer-${i.type}"><div class="market-icon">${i.type === "weapon" ? "†" : i.type === "armor" ? "▣" : i.type === "food" ? "▰" : i.type === "tools" ? "⌁" : "✚"}</div><span class="eyebrow">${i.type.toUpperCase()}${i.quality ? ` · QUALITY ${i.quality}` : ""}</span><h2>${h(i.name)}</h2><p>${h(i.description)}</p><div><b>${i.cost} crowns</b><button class="primary-button" data-buy-offer="${i.id}" ${e.crowns < i.cost ? "disabled" : ""}>Purchase</button></div></article>`).join("") : '<div class="empty-state"><h2>The useful stock is gone.</h2><p>Travel or wait for the market to change.</p></div>'}</div>`
        : `<div class="content-heading"><div><span class="eyebrow">LOCAL MARKET</span><h2>No chartered trade in ${h(t.name)}</h2></div></div><div class="empty-state"><h2>Only quiet bargains are made here.</h2><p>Travel to a settlement with a market to buy supplies and equipment refits.</p></div>`;
    }
    renderCompanyView() {
      const e = this.campaign,
        t =
          e.roster.find((a) => a.id === this.selectedFighterId) ?? e.roster[0];
      return `<div class="content-heading"><div><span class="eyebrow">COMPANY ROSTER</span><h2>${e.roster.filter((a) => a.alive).length} blades under contract</h2></div><div class="heading-stat"><span>DAILY WAGES</span><b>${j(e)} crowns</b></div></div>
      <div class="company-workspace">
        <section class="roster-grid">
          ${e.roster
            ...
            <div class="roster-copy"><span>LEVEL ${a.level} · ${h(a.background)}</span><strong>${h(a.name)}</strong><small>${a.alive ? h(a.epithet) : "Fallen in company service"}</small></div>
            <div class="readiness-ring" style="--ready:${F(a)}"><b>${F(a)}</b><small>READY</small></div>
    ...
    renderFighterDetail(e) {
      ...
      <div class="fighter-sheet-head"><div class="large-portrait">${
        e.alive
          ? e.name
              ...
              .slice(0, 2)
          : "†"
      }</div><div><span class="eyebrow">${h(e.background)} · LEVEL ${e.level}</span><h2>${h(e.name)}</h2><p>${h(e.epithet)}</p></div></div>
      ...
      <section class="injury-section"><span>INJURIES · ARMOR QUALITY ${e.armorQuality}</span>${e.injuries.length ? e.injuries.map((i) => `<b>${h(i)}</b>`).join("") : `<small>${e.alive ? "Fit for service" : "No treatment can be given"}</small>`}</section>
      <section class="perk-section"><div class="section-label"><span>FIELD PERKS</span><b>${e.perkPoints} point${e.perkPoints === 1 ? "" : "s"} available</b></div><div class="perk-list">${e.perks.map((i) => `<span title="${h(D[i].description)}">${h(D[i].name)}</span>`).join("") || "<small>No field specialisation chosen.</small>"}</div>${
        e.perkPoints > 0
          ? `<div class="perk-choices">${Object.keys(D)
              .filter((i) => !e.perks.includes(i))
              .map(
                (i) =>
                  `<button data-choose-perk="${i}" data-perk-fighter="${e.id}"><b>${D[i].name}</b><small>${D[i].description}</small></button>`,
              )
              .join("")}</div>`
          : ""
      }</section>
    ...
    renderContracts() {
      const e = this.campaign;
      return `<div class="content-heading"><div><span class="eyebrow">CONTRACT BOARD · ${S[e.currentLocation].name.toUpperCase()}</span><h2>Work offered where the company stands</h2></div><div class="heading-stat"><span>THREAT / RENOWN</span><b>${e.threat}% / ${e.renown}</b></div></div>
      <div class="contract-grid">${e.contracts
        .map(
          (t) => `<article class="contract-card danger-${t.danger}">
        <div class="contract-seal">${t.danger}</div><span class="eyebrow">${h(t.employer)} · EXPIRES DAY ${t.expiresDay}</span><h2>${h(t.title)}</h2><p>${h(t.briefing)}</p>
        <div class="contract-facts"><div><span>OBJECTIVE · ${t.battleObjective.toUpperCase()}</span><b>${h(t.objective)}</b></div><div><span>TERRAIN</span><b>${h(t.terrainLabel)}</b></div><div><span>ENEMY / DANGER</span><b>${t.enemyFaction.replace("-", " ")} · ${"◆".repeat(t.danger)}${"◇".repeat(3 - t.danger)}</b></div><div><span>PAYMENT</span><b>${t.reward} crowns</b></div></div>
      ...
      <div class="recruit-grid">${
        ...
        <div class="recruit-portrait">${a.name
          ...
          .slice(
            0,
            2,
          )}</div><span class="eyebrow">${h(a.background)}</span><h2>${h(a.name)}</h2><p>${h(a.epithet)}</p>
        ...
        <div class="recruit-kit"><span>${k[a.weaponId].icon}</span><b>${k[a.weaponId].name}${a.hasShield ? " + shield" : ""}</b></div>
    ...
    renderChronicle() {
      ...
      <div class="chronicle-layout"><section class="chronicle-list">${e.history
        ...
        .map(
          (a, i) =>
            `<article><span>${String(e.history.length - i).padStart(2, "0")}</span><p>${h(a)}</p></article>`,
        )
        .join("")}</section>
      <aside class="grave-ledger"><span class="eyebrow">THE DEAD</span>${t.length ? t.map((a) => `<div><b>${h(a.name)}</b><span>${h(a.background)} · ${a.battles} battles · ${a.kills} kills</span></div>`).join("") : "<p>No names entered. The page will not remain clean.</p>"}</aside></div>`;
    }
    bindCampaignControls() {
      ...
      (document
        ...
        document.querySelectorAll("[data-equip-weapon]").forEach((t) =>
          t.addEventListener("click", () => {
            const a = vt(e, t.dataset.equipFighter, t.dataset.equipWeapon);
            this.openCampaign(
              e,
              "company",
              a
        ...
        document.querySelectorAll("[data-toggle-shield]").forEach((t) =>
          t.addEventListener("click", () => {
            const a = bt(e, t.dataset.toggleShield);
            this.openCampaign(
              e,
              "company",
        ...
        document.querySelectorAll("[data-choose-perk]").forEach((t) =>
          t.addEventListener("click", () => {
            const a = mt(e, t.dataset.perkFighter, t.dataset.choosePerk);
            this.openCampaign(
              e,
              "company",
              a
        ...
        document.querySelectorAll("[data-accept-contract]").forEach((t) =>
          t.addEventListener("click", () => {
            const a = e.contracts.find(
              (i) => i.id === t.dataset.acceptContract,
            );
            a && this.showSquadSelection(a);
          }),
        ),
        document
          .querySelector("#rest-company")
          ?.addEventListener("click", () => {
            const t = yt(e);
    ...
    showCampaignEvent(e = "") {
      ...
      ((document.querySelector("#app").innerHTML =
        `<main class="event-screen event-${a.illustration}"><div class="event-vignette"><div class="event-art"><span>${a.illustration === "road" ? "⌁" : a.illustration === "camp" ? "△" : a.illustration === "ruin" ? "▥" : "ϟ"}</span><i></i></div><section class="event-paper"><span class="eyebrow">ROAD EVENT · DAY ${t.day}</span><h1>${h(a.title)}</h1><p>${h(a.body)}</p>${e ? `<small class="travel-arrival">${h(e)}</small>` : ""}<div class="event-choices">${a.choices.map((i) => `<button data-event-choice="${i.id}"><b>${h(i.label)}</b><span>${h(i.detail)}</span></button>`).join("")}</div></section></div></main>`),
        document.querySelectorAll("[data-event-choice]").forEach((i) =>
          i.addEventListener("click", () => {
            const n = pt(t, i.dataset.eventChoice);
            (I(t), this.openCampaign(t, "map", n));
          }),
        ));
    }
    showSettings() {
    ...
    showCampaignEnding() {
      const e = this.campaign,
        t = e.status === "victory";
      ((document.querySelector("#app").innerHTML =
        `<main class="ending-screen ${t ? "victory" : "defeat"}"><section class="ending-card"><div class="ending-seal">${t ? "IP" : "†"}</div><span class="eyebrow">${t ? "THE FRONTIER REMEMBERS" : "THE LEDGER CLOSES"}</span><h1>${t ? "The road bears your name" : "Iron paid, company broken"}</h1><p>${h(e.endingReason)}</p><div class="ending-stats"><div><span>DAYS</span><b>${e.day}</b></div><div><span>VICTORIES</span><b>${e.victories}</b></div><div><span>DEAD</span><b>${e.roster.filter((a) => !a.alive).length}</b></div><div><span>RENOWN</span><b>${e.renown}</b></div><div><span>CROWNS</span><b>${e.crowns}</b></div></div><blockquote>“Every road has a toll. Every victory names it.”</blockquote><button id="ending-ledger" class="primary-button">Return to campaign ledger</button></section></main>`),
        document
    ...
    showSquadSelection(e) {
      const t = this.campaign,
        ...
        <header class="squad-header"><button id="back-to-contracts" class="back-button">← Contract board</button><div><span class="eyebrow">DEPLOYMENT ROSTER</span><h1>${h(e.title)}</h1></div><div class="squad-count"><span>SELECTED</span><b>${i.size}/6</b></div></header>
        <section class="squad-brief"><div><span>EMPLOYER</span><b>${h(e.employer)}</b></div><div><span>OBJECTIVE</span><b>${h(e.objective)}</b></div><div><span>TERRAIN</span><b>${h(e.terrainLabel)}</b></div><div><span>PAYMENT</span><b>${e.reward} crowns</b></div><div><span>DANGER</span><b>${"◆".repeat(e.danger)}${"◇".repeat(3 - e.danger)}</b></div></section>
    ...
    showAftermath(e) {
      ...
      <section class="aftermath-paper">
        <span class="eyebrow">${e.victory ? "CONTRACT SETTLED" : "CONTRACT FAILED"}</span><h1>${h(e.contractTitle)}</h1><p class="aftermath-lead">${e.victory ? "The employer's seal is set. Count what was earned, then count what it cost." : "No payment is due. The company carries its losses back to the road."}</p>
        ...
        <div class="aftermath-columns">
          <section><h2>People</h2>${e.casualties.length ? e.casualties.map((a) => `<p class="casualty"><b>† ${h(a)}</b><span>Killed in company service</span></p>`).join("") : '<p class="clean-result">No company dead.</p>'}${e.routed.map((a) => `<p><b>${h(a)}</b><span>Routed but returned</span></p>`).join("")}${e.injuries.map((a) => `<p><b>${h(a.name)}</b><span>${h(a.injury)}</span></p>`).join("")}${e.levelUps.map((a) => `<p class="level-up"><b>${h(a)}</b><span>Level gained · perk choice available</span></p>`).join("")}</section>
          <section><h2>Salvage</h2>${e.loot.length ? e.loot.map((a) => `<p><b>${h(a)}</b><span>Added to company stores</span></p>`).join("") : '<p class="clean-result">Nothing worth carrying.</p>'}<p><b>Replay verified</b><span>State ${e.hash}</span></p></section>
        </div>
